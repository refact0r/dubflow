...
class LockInDubsVision:
    ...
    def processing_loop(self) -> None:
        """Background thread for frame processing (AWS Rekognition, etc.)"""
        ...
        while self.running:
            if self.frame_processed:
                ...
                # Process frame (this can be slow due to AWS calls)
    ...
    def main_loop(self) -> None:
        """Main event loop for frame capture and display (runs on main thread for macOS compatibility)."""
        ...
        while self.running:
            try:
                ...
                # Show debug visualization if enabled (MUST be on main thread for macOS)
                if Config.SHOW_VIDEO_FEED:
                    debug_frame = self.focus_detector.draw_debug_info(
                        self.frame, self.userFocusState, self.opencv_context
                    )
                    ...
                    # Small delay if not showing video to prevent busy waiting
                    time.sleep(0.01)
    ...
    def start(self) -> None:
        ...
        # Start main loop on main thread (required for macOS cv2.imshow)
        self.running = True
        self.main_loop_encapsulator()
    ...
    def print_session_stats(self) -> None:
        ...
        focus_percentage = (
            self.stats["focused_frames"] / max(1, self.stats["total_frames"])
        ) * 100
        ...
        print(
            f"Average FPS: {self.stats['total_frames'] / max(1, session_duration):.1f}"
        )
        print("=" * 50)
